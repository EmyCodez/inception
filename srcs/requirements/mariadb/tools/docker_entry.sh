...
mysqld_safe &
mysql_pid=$!
# starts MYSQL server in the safe mode and restarts the server if its crashes
# using "$" the server runs in the background
# captures the process ID using "$!" of the MYSQL server and store them in teh mysql_pid
...
# it keeps looping the server printing dots ever 0.2 seconds until the server responds
